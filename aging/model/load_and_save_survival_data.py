...
def save_predictions_to_csv(predicts_df, step, target, organ, view, transformation, model_name, fold, best_params):
    ...
    filename = 'PredictionsSurvival_' + organ + '_' view + '_' + transformation + '_' +  target + '_' + model_name + '_' + hyper_parameters_name + '_' + str(fold) + '_' + step + '.csv'
    predicts_df.set_index('id').to_csv(path_predictions_survival + '/' + filename)

def save_predictions_regression_to_csv(predicts_df, step, target, organ, view, transformation, model_name, fold, best_params):
    hyper_parameters_name = '_'.join([str(elem) for elem in best_params])
    if len(best_params) != 7:
        hyper_parameters_name = hyper_parameters_name + '_' + '_'.join(['NA' for elem in range(7 - len(best_params))])
    filename = 'PredictionsSurvivalRegression_' + organ + '_' view + '_' + transformation + '_' +  target + '_' + model_name + '_' + hyper_parameters_name + '_' + str(fold) + '_' + step + '.csv'
