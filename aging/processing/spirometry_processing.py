...
def read_spirometry_data(**kwargs):
	...
	for instance in [0, 1, 2, 3]:
	    ...
	    temp = pd.read_csv(path_data,  nrows = nrows, usecols = ['20151-%s.0'% instance] + [elem + str(int_) for elem in cols for int_ in range(3)] +  ['eid', '21003-%s.0'% instance, '31-0.0']).set_index('eid')
