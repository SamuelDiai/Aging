...
do
	...
	do
		...
		# 			out_file="./logs/${target}_${model}_${dataset}_${fold}.out"
		# 			err_file="./logs/${target}_${model}_${dataset}_${fold}.err"
		# 			ID=$(sbatch --parsable --dependency=afterok:$ID_raw --error=$err_file --output=$out_file --job-name=$job_name --mem-per-cpu=$memory -c $n_cores -p medium -t 4-23:59 batch_jobs/predictions/single.sh $model $outer_splits $inner_splits $n_iter $target $dataset $fold)
		# 			IDs+=($ID)
		# 		done
		#
		# 		if [ $model != "NeuralNetwork" ]
